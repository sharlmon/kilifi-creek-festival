...
const screeningParts = computed(() => {
  const html = content.screenings.html.replace(/<div class="bounded">\s*<img[^>]*alt="KCF 2025 Programming"[^>]*>\s*<\/div>/, '')
...
<template>
  ...
  <div v-else-if="page" class="original-content interior-content" :class="'page-'+slug" @submit="submit" v-html="responsiveHtml(page.html)"></div>
</template>
